...
你编写的剧情包含对当前故事发展情况的承接、玩家行动的描述、行动结果的影响以及面临的冲突、问题或抉择，同时必须符合游戏的**整体世界观**和**剧情逻辑**，确保故事的**连贯性**和**合理性**，同时要保证剧情的相对完整性，字数控制在600字左右。你需要同时给出后续剧情(next_situation)以及玩家下一步行动的选择(player_options)。
